...
START_TIME=$(date +%s)   # Phase 3 시작 전 기록 — DURATION 계산에 사용
mkdir -p "$REPORT_DIR/logs"
mkdir -p "$REPORT_DIR/screenshots"
mkdir -p "$REPORT_DIR/scripts"
# plan-report.html       → $REPORT_DIR/plan-report.html
# logs/                  → 단위/API/E2E 실행 로그
# screenshots/           → E2E 스크린샷 (실패 캡처 포함)
# scripts/               → 테스트 중 생성된 임시 스크립트
```
...
**단위/통합:**
```bash
# 로그는 $REPORT_DIR/logs/unit.log 에 저장
...
# TC별 응답을 logs/api-TC-001.log 에 저장
curl -s -w "\n%{http_code}" \
  -H "Authorization: Bearer $TOKEN" \
  "$API_BASE_URL/endpoint" \
  2>&1 | tee "$REPORT_DIR/logs/api-TC-001.log"

# API 테스트에 사용한 curl 명령은 scripts/api-TC-001.sh 로 저장
```

**E2E:**
```bash
# playwright: 스크린샷을 $REPORT_DIR/screenshots/ 에 저장
npx playwright test --reporter=json \
  --screenshot=only-on-failure \
  --output="$REPORT_DIR/screenshots" \
  2>&1 | tee "$REPORT_DIR/logs/e2e.log"

# cypress: 스크린샷/비디오를 $REPORT_DIR/screenshots/ 에 저장
...
**임시 스크립트 저장 규칙:**
- 테스트 중 생성하는 curl 명령, 검증 스크립트, seed 스크립트는 `$REPORT_DIR/scripts/` 에 저장
- 파일명 형식: `{TC번호}-{목적}.sh` (예: `TC-003-create-user.sh`, `TC-007-seed-data.sh`)
...
```bash
# GitHub (pr-comment.md의 heredoc 방식 사용)
...
# GitLab: pr-comment.md의 curl MR Note API 방식 사용
```
